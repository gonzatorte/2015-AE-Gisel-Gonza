...
skeleton newGA
{
	...
	Problem::Problem ():_dimension(0),_costo_objetivo(0),_costo_aperitivos(NULL)
	{}
	...
	ostream& operator<< (ostream& os, const Problem& pbm)
	{
		...
		os<<"Costos aperitivos: "<<endl<<endl;
		for (int i=0;i<pbm._dimension;i++){
			os<<pbm._costo_aperitivos[i]<<",";
		}
		os<<endl;
		os<<"Costos objetivo: " << pbm._costo_objetivo <<endl<<endl;
		os<<endl;
		return os;
	}
	...
	istream& operator>> (istream& is, Problem& pbm)
	{
		...
		is.getline(buffer,MAX_BUFFER,'\n');
		sscanf(buffer,"%f",&pbm._costo_objetivo);
		pbm._costo_aperitivos = new float [pbm._dimension]; 
		for (int i=0; i<pbm._dimension; i++){
			is.getline(buffer,MAX_BUFFER,'\n');
			sscanf(buffer,"%f",&(pbm._costo_aperitivos[i]));
		}

		return is;
	}

	float * Problem::costo_aperitivos() const{
		return _costo_aperitivos;
	}

	float Problem::costo_objetivo() const{
		return _costo_objetivo;
	}
	...
	double Solution::_fitness_fun(Rarray<int> & genoma, float * costos, float obj)
	{
		...
		double distancia_objetivo = abs(costo_total - obj);
	...
	{
		float costos [] = {15.2};
		Rarray<int> genoma = Rarray<int>(1);
		genoma[0] = 1;
		cout << "B Testing: " << endl;
...
// StopCondition_1 -------------------------------------------------------------------------------------
	...
	bool StopCondition_1::EvaluateCondition(const Problem& pbm,const Solver& solver,const SetUpParams& setup)
	{
		...
		//		cout << "Aperitivo " << i << ":" << solver.best_solution_trial().var(i) << endl;
		//	}
		//}
		return (fin);
	}
